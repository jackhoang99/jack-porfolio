      ...
      <ul className="space-y-6 md:space-y-4">
        {navItems.map(({ id, label }) => (
          <li key={id}>
            ...
            >
              <span className={`hidden md:block mr-2 text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity ${darkMode ? 'text-white' : ''}`}>
                {label}
              </span>
              <div className={`h-1.5 w-1.5 md:h-2 md:w-2 rounded-full border transition-all duration-300 ${
                activeSection === id 
